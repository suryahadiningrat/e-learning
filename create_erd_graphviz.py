...
def create_erd_graphviz():
    """Create ERD using Graphviz DOT language for better layout"""
...
'''
    ...
    # Write DOT file
    with open('erd_graphviz.dot', 'w', encoding='utf-8') as f:
        f.write(dot_content)
    
    return 'erd_graphviz.dot'

def generate_erd_images(dot_file):
    """Generate PNG and PDF from DOT file using Graphviz"""
    try:
        # Check if Graphviz is installed
        result = subprocess.run(['which', 'dot'], capture_output=True, text=True)
        if result.returncode != 0:
            print("Graphviz tidak terinstall. Menginstall Graphviz...")
            # Try to install Graphviz using Homebrew
            subprocess.run(['brew', 'install', 'graphviz'], check=True)
        
        # Generate PNG
        subprocess.run([
            'dot', '-Tpng', '-Gdpi=300', 
            dot_file, '-o', 'erd_graphviz_clean.png'
        ], check=True)
        
        # Generate PDF
        subprocess.run([
            'dot', '-Tpdf', 
            dot_file, '-o', 'erd_graphviz_clean.pdf'
        ], check=True)
        
        print("ERD berhasil dibuat dengan Graphviz:")
        print("- erd_graphviz_clean.png")
        print("- erd_graphviz_clean.pdf")
        
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error saat menjalankan Graphviz: {e}")
        return False
    except FileNotFoundError:
        print("Graphviz tidak ditemukan. Silakan install terlebih dahulu:")
        print("brew install graphviz")
        return False

if __name__ == "__main__":
    print("Membuat ERD menggunakan Graphviz...")
    dot_file = create_erd_graphviz()
    success = generate_erd_images(dot_file)
    
    if not success:
        print("\\nJika Graphviz gagal, akan menggunakan metode alternatif...")
        # Fallback to matplotlib with better spacing
        print("Menggunakan matplotlib dengan spacing yang lebih baik...")