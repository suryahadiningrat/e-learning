...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="<?= base_url('admin/materi/update/' . $materi['id']) ?>" method="post" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="jadwal_id" name="jadwal_id" required>
                                ...
                                <?php foreach ($jadwal as $jadwal_item): ?>
                                    <option value="<?= $jadwal_item['id'] ?>" <?= old('jadwal_id', $materi['jadwal_id']) == $jadwal_item['id'] ? 'selected' : '' ?>>
                                        <?= $jadwal_item['nama_mata_pelajaran'] ?> - <?= $jadwal_item['nama_kelas'] ?> (<?= $jadwal_item['hari'] ?>, <?= $jadwal_item['jam_mulai'] ?>-<?= $jadwal_item['jam_selesai'] ?>) - <?= $jadwal_item['nama_guru'] ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
