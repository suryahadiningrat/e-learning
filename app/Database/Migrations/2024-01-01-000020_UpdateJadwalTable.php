...
class UpdateJadwalTable extends Migration
{
    public function up()
    {
        ...
        // Update data jadwal yang ada dengan mapping mata pelajaran
        $mapping = [
            'Matematika' => 1,
            'Bahasa Indonesia' => 2,
            'Bahasa Inggris' => 3,
        ...
        foreach ($mapping as $nama => $id) {
            $this->db->query("UPDATE jadwal SET mata_pelajaran_id = ? WHERE mata_pelajaran = ?", [$id, $nama]);
        }
        
        // Hapus kolom mata_pelajaran lama
        $this->forge->dropColumn('jadwal', 'mata_pelajaran');
    }
    ...
    public function down()
    {
        ...
        foreach ($mapping as $id => $nama) {
            $this->db->query("UPDATE jadwal SET mata_pelajaran = ? WHERE mata_pelajaran_id = ?", [$nama, $id]);
        }
        
        // Hapus foreign key dan kolom mata_pelajaran_id
        $this->forge->dropForeignKey('jadwal', 'jadwal_mata_pelajaran_id_foreign');
        $this->forge->dropColumn('jadwal', 'mata_pelajaran_id');
    }
} 